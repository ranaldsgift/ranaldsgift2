...
<div class="inventory-item-display-container {itemBuild.weapon ? 'weapon' : 'jewelry'}">
	<div class="inventory-item-header border-01" style="--gradientColor: {gradientColor}; --textColor: {textColor};">
		<span class="[text-shadow:_1px_1px_0_rgb(0_0_0_/_40%)]">{itemBuild?.weapon?.name ?? itemType.toString()}</span>
	</div>
	<div class="inventory-item-summary-container border-01" data-rarity={itemBuild.rarity}>
		<div class="item-power-level"><input type="number" value={itemBuild.powerLevel} min="0" max="300" /></div>
	...
	{#if itemBuild.weapon?.tooltip}
		<p class="inventory-item-footer px-2 border-01">
