...
class BuildHelper {
	static getBuildFromSearchParams(searchParams: URLSearchParams): ICareerBuild | null {
		const build: ICareerBuild = {} as ICareerBuild;

		const careerId = searchParams.get("career");
		if (careerId) {
			const career = StaticData.careers.find((career) => career.id === parseInt(careerId));
			...
			if (primaryWeaponParams?.split("-").length === 4) {
				...
				if (secondaryWeaponParams?.split("-").length === 4) {
					...
					const secondaryWeaponProperty1 = secondaryWeapon.properties.find(
						(property) => property.id === secondaryWeaponProperty1Id
					);
					const secondaryWeaponProperty2 = secondaryWeapon.properties.find(
						(property) => property.id === secondaryWeaponProperty2Id
					);
				...
				if (necklaceParams?.split("-").length === 3) {
					...
					const necklaceProperty1 = StaticData.properties.find((property) => property.id === necklaceProperty1Id);
					const necklaceProperty2 = StaticData.properties.find((property) => property.id === necklaceProperty2Id);
					const necklaceTrait = StaticData.traits.find((trait) => trait.id === necklaceTraitId);
				...
				if (charmParams?.split("-").length === 3) {
					...
					const charmProperty1 = StaticData.properties.find((property) => property.id === charmProperty1Id);
					const charmProperty2 = StaticData.properties.find((property) => property.id === charmProperty2Id);
					const charmTrait = StaticData.traits.find((trait) => trait.id === charmTraitId);
				...
				if (trinketParams?.split("-").length === 3) {
					...
					const trinketProperty1 = StaticData.properties.find((property) => property.id === trinketProperty1Id);
					const trinketProperty2 = StaticData.properties.find((property) => property.id === trinketProperty2Id);
					const trinketTrait = StaticData.traits.find((trait) => trait.id === trinketTraitId);
