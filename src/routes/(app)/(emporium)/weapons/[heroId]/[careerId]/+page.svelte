...
<Breadcrumb
	links={[
		...
		{ text: data.heroName, href: `/weapons/${data.heroId}` },
	]}
	layout="emporium">{data.careerName}</Breadcrumb
>
		...
		<div class="careers flex flex-col gap-2 w-full max-w-[500px]">
			{#each data.careers as career}
				<a
					...
					class:selected={data.careerId === career.id}
					href={`/weapons/${data.heroId}/${career.id}`}
				>
...
<style>
	...
	.left-container .career-icon-wrapper.selected .career-name,
	.left-container .career-icon-wrapper.selected:hover .career-name {
		color: #fff;
		cursor: default;
	}
	.career-icon-wrapper.selected,
