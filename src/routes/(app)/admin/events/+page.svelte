...
<div class="page-layout border-01 background-39 m-auto max-w-7xl top-left-shadow relative p-4 mobile:p-0">
	<div class="absolute inset-0 bg-gradient-to-b from-black/60 to-black/80 pointer-events-none z-0"></div>
	<div class="page-content mobile:p-5 text-[1.2rem] relative z-1">
		<div class="grid gap-4">
			{#each events as event}
				<div class="grid mobile:grid-cols-5 grid-cols-2 gap-2 items-center">
					<span class="col-span-1">{event.name}</span>
					<span class="col-span-1 hidden mobile:block">{new Date(event.startDate).toLocaleString()}</span>
					<span class="col-span-1 hidden mobile:block">{new Date(event.endDate).toLocaleString()}</span>
					<span class="col-span-1 hidden mobile:block">{event.isActive ? "Active" : "Inactive"}</span>
